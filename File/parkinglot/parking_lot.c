...
void save(char *file,par *arr)
{
	FILE *fptr=NULL;
        fptr=fopen(file,"w");
	fwrite(&amount,sizeof(amount),1,fptr);
	fwrite(arr,sizeof(par),MAX,fptr);
	fclose(fptr);
}
int main(int argc,char *argv[])
{
	...
	FILE *fptr=NULL;
	fptr=fopen(argv[1],"r+");
	fread(&amount,sizeof(amount),1,fptr);
	for(int i=0;i<MAX;i++)
	{
		fread(arr+i,sizeof(par),1,fptr);
	}
	fclose(fptr);
    char choice;
    while(1){
